...
### Hotfix (2026-07-23)
- **Security Hotfix (No Version Bump)** — Untracked and removed `ripple.config.json.bak` from git history and updated `.gitignore` to block `*.bak` files. Sanitized `CHANGELOG.md` to remove specific domain patterns matching private usernames. No version bump was made to keep this as a discrete security hygiene commit before pushing to GitHub.
...
- First consumer: FamilyFirst (labelle.family / familyfirst.site as separate projects, admin `person_search` events, member auto-identify).
